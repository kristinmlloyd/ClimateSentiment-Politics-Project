...
test_df = pd.read_csv("../Data/raw-data/Senators.csv")
...
## Test on President Data

```{python}

# 1. First load necessary libraries and the preprocessing function
import pandas as pd
import re
from nltk.corpus import stopwords
from nltk.tokenize import TreebankWordTokenizer
...
# 2. Load and preprocess training data (hi5.csv) to fit the vectorizer
train_df = pd.read_csv("../Data/raw-data/train.csv")
train_df['message'] = train_df['message'].apply(preprocess)

# 3. Load President.csv
president_df = pd.read_csv("../Data/raw-data/President.csv")
president_df['message'] = president_df['message'].apply(preprocess)

# 4. Create and fit the vectorizer on training data
vector = TfidfVectorizer(ngram_range=(1,20), min_df=2)
vector.fit(train_df['message'])

# 5. Transform President.csv data
president_features = vector.transform(president_df['message'])

# 6. Load the three best models and make predictions
from sklearn.linear_model import LogisticRegression
from sklearn.naive_bayes import MultinomialNB
from sklearn.svm import LinearSVC

# Define and train models on entire hi5.csv dataset
lr_model = LogisticRegression(C=10)
nb_model = MultinomialNB(alpha=1)
svm_model = LinearSVC(C=10, class_weight=None)

# Fit models
lr_model.fit(vector.transform(train_df['message']), train_df['sentiment'])
nb_model.fit(vector.transform(train_df['message']), train_df['sentiment'])
svm_model.fit(vector.transform(train_df['message']), train_df['sentiment'])

president_df['LogisticRegression_predictions'] = lr_model.predict(president_features)
president_df['NaiveBayes_predictions'] = nb_model.predict(president_features)
president_df['LinearSVM_predictions'] = svm_model.predict(president_features)

selected_columns = ['LogisticRegression_predictions', 'NaiveBayes_predictions', 'LinearSVM_predictions']
president_df['model_label'] = president_df[selected_columns].mode(axis=1).iloc[:,0]

for col in selected_columns:
    president_df = president_df.drop(col, axis=1)

president_df.to_csv("../Data/processed-data/President_sentiment.csv", index=False)

if 'sentiment' in president_df.columns:
    matches = (president_df['model_label'] == president_df['sentiment']).sum()
    total = len(president_df)
    accuracy = matches / total * 100
    ...
    print("\nConfusion Matrix:")
    print(pd.crosstab(president_df['sentiment'], president_df['model_label'], margins=True))
