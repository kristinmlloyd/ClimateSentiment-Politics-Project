...
df <- df %>%
  mutate(state = tolower(state.name[match(state, state.abb)]))  # Match state abbreviations to full names

state_senators <- df %>%
  distinct(state, party) %>%  # Ensure unique combinations of state and party
  group_by(state) %>%
  summarise(
